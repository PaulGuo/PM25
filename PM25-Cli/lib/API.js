...
API.prototype.gracefulReload = function(process_name, opts, cb) {
  ...
  //Common.printOut(conf.PREFIX_MSG_WARNING + chalk.bold.yellow('Use http://pm2.PM25.io/docs/usage/signals-clean-restart/ instead'));
  ...
  that.Client.executeRemote('getMonitorData', {}, function(err, list) {
    ...
    if (process.stdout.isTTY === false) {
      ...
      if (that.gl_interact_infos) {
        Common.printOut(chalk.green.bold('●') + ' Agent online - public key: %s - machine name: %s - Web access: https://app.PM25.io/', that.gl_interact_infos.public_key, that.gl_interact_infos.machine_name);
      }
