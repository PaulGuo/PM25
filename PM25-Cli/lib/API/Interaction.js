...
module.exports = function(CLI) {
  ...
  CLI.prototype._pre_interact = function(secret_key, public_key, machine, opts) {
    ...
    if (secret_key == 'stop' || secret_key == 'kill') {
      console.log(chalk.cyan('[PM25.io]') + ' Stopping agent...');
      that.killInteract(function() {
        console.log(chalk.cyan('[PM25.io]') + ' Stopped');
        return process.exit(cst.SUCCESS_EXIT);
      });
      return false;
    }
    if (secret_key == 'info') {
      console.log(chalk.cyan('[PM25.io]') + ' Getting agent information...');
    ...
    if (secret_key == 'delete') {
      that.killInteract(function() {
        try {
          ...
          console.log(chalk.cyan('[PM25.io]') + ' No interaction config file found');
          return process.exit(cst.SUCCESS_EXIT);
        }
        console.log(chalk.cyan('[PM25.io]') + ' Agent interaction ended');
        return process.exit(cst.SUCCESS_EXIT);
      });
      return false;
    }
    ...
    if (secret_key && !public_key) {
      console.error(chalk.cyan('[PM25.io]') + ' Command [%s] unknown or missing public key', secret_key);
